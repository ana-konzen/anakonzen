    ...
    >
      <ProjectSection>
        <div className="w-full justify-between md:flex-row flex-col flex space-y-4 md:space-y-0">
          ...
          {project.medium && (
            <ProjectDetails title="Made with" data={project.medium} />
          )}
        </div>
      </ProjectSection>
      ...
      {project.content &&
        project.content.map((item: SanityDocument) => (
          <ProjectContentSection key={item._key} item={item} />
        ))}
    </motion.div>
  );
}
...
export function ProjectContentSection({ item }: { item: SanityDocument }) {
  ...
  const contClass = classNames({
    "flex flex-col py-8 md:items-center md:flex-row space-y-12 md:space-x-16":
      !item.vertical,
  ...
  const components: PortableTextComponents = {
    block: {
      h5: ({ children }) => (
        <h5 className="font-semibold text-sm uppercase my-8 mb-2">
          {children}
        </h5>
      ),
    },
      ...
      link: ({ value, children }) => {
        ...
        return (
          <a
            href={value?.href}
            target={target}
            className="font-medium hover:font-bold mt-8 block"
          >
            {children} →
          </a>
        );
    ...
    >
      {(item.title || item.body) && (
        <div className={`text-base max-w-80 font-sans ${textClass}`}>
          {item.title && (
            <h3 className="font-bold text-sm uppercase mb-2">{item.title}</h3>
          )}
