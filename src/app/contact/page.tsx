...
export default function ContactPage() {
  return (
    <>
      <AboutPageSketch />
      <div className="relative top-8 flex text-sm flex-col items-center justify-center w-full h-full font-mono ">
        <p className="mb-20 max-w-[400px]">Please email me to get in contact!</p>
...
function ContactItem({ title, href }: { title: string; href: string }) {
  return (
    <div className="mb-8 max-w-[400px]">
      <div className="flex flex-col space-y-4 uppercase text-center items-center w-[400px] font-bold ">
        <a className="hover:bg-amber-200" href={href} target="_blank">
          {title}
        </a>
        <Line length={100} ver={true} />
      </div>
    </div>
  );
}
